...
def create_and_train_model(X_train, y_train, X_test, y_test):
    """
    Defines, compiles, and trains a basic CNN model for classification.
...
def preprocess_uploaded_image(image_file):
    ...
    Resize and normalize it while ensuring proper reshaping.
    """
    try:
        ...
        st.write("Shape after normalization:", image.shape)

        # Reshape to fit the CNN input format
        image = image.reshape(1, 128, 128, 3)  # Explicit reshape to CNN's expected shape
        st.write("Final preprocessed image shape:", image.shape)
